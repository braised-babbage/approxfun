...
(deftest test-operator-arithmetic ()
  "Check that we can perform basic arithmetic with operators."
  (flet ((check= (a b)
           (ap::randomized-equality-check a b)))
    (multiple-value-bind (omega x) (ap:make-domain -1 1)
      (let* ((I (ap:I omega))
        ...
        (is (check= x (ap:@ I x)))
        ;; operator & scalar
        (is (check= 3x (ap:@ (ap:+ I 2) x)) "(I+2)x = 3x")
        (is (check= 3x (ap:@ (ap:+ 2 I) x)) "(2+I)x = 3x")
        (is (check= -x (ap:@ (ap:- I 2) x)) "(I-2)x = -x")
        (is (check= x (ap:@ (ap:- 2 I) x)) "(2-I)x = x")
        (is (check= x (ap:@ (ap:* I 2) x/2)) "(I*2)(x/2) = x")
        (is (check= x (ap:@ (ap:* 2 I) x/2)) "(2*I)(x/2) = x")
        (is (check= x/2 (ap:@ (ap:/ I 2) x)) "(I/2)x = x/2")
        (signals error (ap:/ 2 I))
        ;; operator & approxfun
        (is (check= x^2+x (ap:@ (ap:+ I x) x)) "(I+x)x = x^2+x")
        (is (check= x^2+x (ap:@ (ap:+ x I) x)) "(x+I)x = x^2+x")
        (is (check= x-x^2 (ap:@ (ap:- I x) x)) "(I-x)x = x-x^2")
        (is (check= x^2-x (ap:@ (ap:- x I) x)) "(x-I)x = x^2-x")
        (is (check= x^2 (ap:@ (ap:* I x) x)) "(I*x)x = x^2")
        (is (check= x^2 (ap:@ (ap:* x I) x)) "(x*I)x = x^2")
        (is (check= (ap:/ x 1+x^2)
                    (ap:@ (ap:/ I 1+x^2) x)) "(I/(1+x^2))x = x/(1+x^2)")
        (signals division-by-zero (ap:/ I x))
        (signals error (ap:/ x I))
        ;; operator & operator
        (is (check= x (ap:@ (ap:+ I I) x/2)) "(I+I)x/2 = x")
